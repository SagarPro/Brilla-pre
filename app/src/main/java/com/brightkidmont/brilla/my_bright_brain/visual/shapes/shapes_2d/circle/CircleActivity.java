...
public class CircleActivity extends AppCompatActivity {
    ...
    public static String circleTab;
    private int i = 0, p = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        q = MediaPlayer.create(getBaseContext(), R.raw.circle1);
        ...
        viewTreeObserver1.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                animation = new TranslateAnimation(-width*8,-width*7,0,0);
        ...
        //assigning respective audio files for each tab and positioning bottom imageView in respective position
        tabLayout.setOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            @Override
            public void onTabSelected(TabLayout.Tab tab) {
                circleTab = tab.getText().toString();
                switch (circleTab){
                    case "1":
                        stopAllAudio();
                        q = MediaPlayer.create(getBaseContext(), R.raw.circle1);
                    ...
                    case "2":
                        stopAllAudio();
                        q = MediaPlayer.create(getBaseContext(), R.raw.circle2);
                        break;
                    case "3":
                        stopAllAudio();
                        q = MediaPlayer.create(getBaseContext(), R.raw.circle3);
                        break;
                    case "4":
                        stopAllAudio();
                        q = MediaPlayer.create(getBaseContext(), R.raw.circle4);
                        break;
                    case "5":
                        stopAllAudio();
                        q = MediaPlayer.create(getBaseContext(), R.raw.circle5);
                        break;
                    case "6":
                        stopAllAudio();
                    ...
                    case "7":
                        stopAllAudio();
                        q = MediaPlayer.create(getBaseContext(), R.raw.circle7);
                        break;
                    case "8":
                        stopAllAudio();
                ...
                if(i<p){
                    switch (p){
                        case 1:
                            animation = new TranslateAnimation(x,-width*7,0,0);
                            x = -width*7;
                            break;
                        case 2:
    ...
    private void setupViewPager(ViewPager viewPager) {
        ...
        adapter.addFrag(new Circle1(), "1");
        adapter.addFrag(new Circle2(), "2");
        adapter.addFrag(new Circle3(), "3");
        adapter.addFrag(new Circle4(), "4");
        adapter.addFrag(new Circle5(), "5");
        adapter.addFrag(new Circle6(), "6");
        adapter.addFrag(new Circle7(), "7");
        adapter.addFrag(new Circle8(), "8");
        viewPager.setAdapter(adapter);
    }

    //Stopping all audio files
